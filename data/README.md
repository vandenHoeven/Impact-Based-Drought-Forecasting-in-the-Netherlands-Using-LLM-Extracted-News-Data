...
All pipeline data lives under `chapters/*/data/` (and chapter `results/`). This folder only holds this overview — there is no shared root-level `raw/` or `processed/` tree.
...
- Chapter 09 NL meteo clip (`meteo_nl/`), processed panels/manifests, frozen AutoML results
...
| 09 | DEM elevation raster | Digital elevation model tile (upstream provider not documented in-repo) | ~125 MB | **No** (size) | `chapters/09_baseline_forecasting/data/raw/elevation/` |
...
| 09 | Regenerable EDA / QA galleries | Generated diagnostics from statics / predictor notebooks | varies | **No** (local only) | `…/data/processed/qa_predictor_july_maps/`, `soil_eda/`, `images_for_elevation_eda/` |
