...
All pipeline data lives under `chapters/*/data/` (and chapter `results/`). This file is the data catalogue at the repo root.

Full Lexis / newspaper article bodies **cannot be redistributed** (copyright). Large rasters and wildfire grids exceed GitHub’s size limits and stay local. A private ~50 GB global meteo archive on the author’s machine was used once to build the small NL clip that *is* shipped; that archive was **never** part of this repository.

## What is / isn’t shipped
...
- Chapter 07 report inputs and frozen `results/` figures/tables
- Chapters 08–09 NL meteo clip (`meteo_nl/`), processed panels/manifests, frozen AutoML results, curated statics figures under `results/figures/statics/`
...
- Chapters 08–09 DEM / BRO / CLC / WorldPop rasters, GPKG caches, regenerable EDA galleries
...
| Chapter | Dataset | Source | Size | In repo? | Path |
| --- | --- | --- | --- | --- | --- |
...
| 08–09 | Global SPEI/SPI / meteo archive (clip source) | External climate archives on the author’s local machine only (never in git, never a repo artifact) | ~50 GB | **No** (external only) | none (set via `CH09_METEO_GLOBAL_DIR`; not in this repo) |
| 08–09 | DEM elevation raster | [Copernicus DEM](https://ec.europa.eu/eurostat/web/gisco/geodata/digital-elevation-model/copernicus#Elevation) (GLO-90, ESA/Airbus Defence and Space, TanDEM-X-derived, DLR), via Eurostat GISCO | ~125 MB | **No** (size) | `chapters/08_09_baseline_forecasting/data/raw/elevation/` |
| 08–09 | BRO soil map | BRO (Dutch Basisregistratie Ondergrond, national soil / subsurface registry) | ~299 MB | **No** (size) | `chapters/08_09_baseline_forecasting/data/raw/soil/` |
| 08–09 | CLC 2018 land cover | Copernicus CORINE Land Cover (CLC2018) | ~197 MB | **No** (size) | `chapters/08_09_baseline_forecasting/data/static/land/` |
| 08–09 | WorldPop population rasters | WorldPop.org gridded population | varies | **No** (size) | `chapters/08_09_baseline_forecasting/data/static/` |
| 08–09 | BRO soil GPKG caches | Derived / cached from the BRO soil map | varies | **No** (rebuildable) | `chapters/08_09_baseline_forecasting/data/processed/*.gpkg` |
| 08–09 | Dev / test panels + manifests | Derived: meteo zonal means + statics + impact labels (`02_data_preparation.ipynb`) | ~78 MB (processed tree; galleries excluded) | **Yes** (panels/manifests) | `chapters/08_09_baseline_forecasting/data/processed/` |
| 08–09 | Curated statics figures | Selected outputs from `soil_eda` / `images_for_elevation_eda` (Ch08–09 statics notebook `01`) | ~0.8 MB (4 PNGs) | **Yes** | `chapters/08_09_baseline_forecasting/results/figures/statics/` |
| 08–09 | Full regenerable EDA / QA galleries | Generated diagnostics from statics / predictor notebooks (bulk; not curated) | varies | **No** (local only) | `…/data/processed/qa_predictor_july_maps/`, `soil_eda/`, `images_for_elevation_eda/` |
| 08–09 | Frozen AutoML results | Optuna model + predictor-subset search (`run_automl.py` / notebook `03`) | small JSON/CSV + figures | **Yes** | `chapters/08_09_baseline_forecasting/results/automl_results/` |
